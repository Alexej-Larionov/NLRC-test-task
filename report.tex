...
\usepackage{graphicx}
\usepackage{float}
\usepackage[a4paper,top=1.2cm,bottom=1.2cm,left=2cm,right=2cm]{geometry}
\makeatletter
...
optimization dynamics are empirically more effective. 
\subsubsection{ 2. AdamW optimizer}
AdamW is an adaptive optimization algorithm based on Adam,
...
updates is beneficial. 
\subsection{ 2. Performance comparison }
\subsubsection{ 1. Data,model and experiment setup} 
To compare the optimizers, we used the
pretrained model
...
computational and memory efficiency across configurations. 
\subsubsection{ 2.Experiment results}
For the first experiment, an effective batch size of
...
\caption{492k effective batch, 5e-5 AdamW lr, 1e-3 Muon lr, wd =
0.01, cosine annealing with \textasciitilde5\% warmup tokens.}
\end{figure}

However, after approximately 66M tokens and until the end of training,
the trend reverses. Muon achieves the lowest loss, while the Mixed
configuration remains intermediate between Muon and AdamW.

\begin{figure}[H]
\centering
\includegraphics{report/attachments/Pasted image 20260427110942.png}
\caption{learning rate slope display for Experiment 1}
\end{figure}
...
\begin{figure}[H]
\includegraphics{report/attachments/Pasted image 20260427105822.png}
\caption{ 82k effective batch, 5e-5 AdamW lr, 1e-3 Muon lr, wd = 0.01, cosine
annealing with \textasciitilde5\% warmup tokens.}
\end{figure}
Compared to Experiment 1, both Muon and Mixed exhibit a positive slope
when exiting the warmup phase. This behavior is likely caused by the
higher update frequency, which introduces noisier gradients.

\begin{figure}[H]
\centering
\includegraphics{report/attachments/Pasted image 20260427110718.png}
\caption{learning rate slope display for Experiment 2}
\end{figure}

For the final experiment, the Muon learning rate was reduced to
...
\caption{82k effective batch, 5e-5 AdamW lr, 1e-5 Muon lr, wd =
0.0, cosine annealing with \textasciitilde5\% warmup tokens.}
\end{figure}

The slope analysis reflects improved stability. Muon no longer exhibits
a strong positive slope after the warmup phase, indicating that the
transition is less noisy. However, this comes at the cost of slower
convergence: the loss decreases more gradually, and Muon does not show
the same rapid recovery as in the higher learning rate setting. Overall,
lowering the learning rate and removing weight decay stabilizes Muon but
reduces its convergence speed, resulting in a more stable yet less
competitive optimization behavior compared to the baseline
configurations.

\begin{figure}[H]
\centering
\includegraphics{report/attachments/Pasted image 20260427111803.png}
\caption{learning rate slope display for Experiment 2}
\end{figure}

\hypertarget{evaluation-results}{%
\subsubsection{3. Evaluation results}\label{evaluation-results}}

The table shows performance on several downstream tasks for different
optimizer setups.
...
The best overall results are achieved by the final \textbf{Muon} run
(with lower learning rate). It performs at the top or near the top on
most tasks. In particular:\\
...
learning rates make it competitive or better than the baseline.

\hypertarget{resource-efficiency-analysis}{%
\subsection{3. Resource efficiency
analysis}\label{resource-efficiency-analysis}}

The first plot shows optimizer step time as a percentage of the total
step time. AdamW introduces very low overhead in all configurations,
typically staying below 1\%. In contrast, Muon adds noticeable overhead
due to the additional Newton--Schulz iterations.

\begin{figure}[H]
\centering
\includegraphics{report/attachments/Pasted image 20260427115756.png}
\caption{optimizer step overhead}
\end{figure}


This overhead depends on the setup: with large batch sizes it remains
relatively moderate, while with smaller batches it becomes significantly
higher, reaching up to around 5\% in the most expensive case. This
indicates that Muon is more efficient when the forward and backward
passes dominate computation, and less efficient when optimizer cost
becomes a larger fraction of the step.

\begin{figure}[H]
\centering
\includegraphics{report/attachments/Pasted image 20260427114533.png}
\caption{optimizer memory usage}
\end{figure}

The second plot shows optimizer state memory usage. AdamW consistently
has the highest memory consumption because it stores both first and
second moment estimates for all parameters. Muon requires substantially
less memory, since it does not maintain full moment statistics in the
same way. Mixed configurations fall between the two, reflecting the
proportion of parameters handled by each optimizer.\\
